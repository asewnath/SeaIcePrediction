...
def create_model():
    ...
    cnn.add(keras.layers.SeparableConv2D(128, (2, 2), activation='relu', padding='same', 
                                  data_format="channels_first"))
    cnn.add(keras.layers.Conv2D(64, (2, 2), activation='relu', padding='same', 
                                  data_format="channels_first"))       
    ...
    model.add(keras.layers.LSTM(units, activation='relu', recurrent_activation='sigmoid', 
...
feat   = np.reshape(feat, (389880, numTimeSeries, numChannels, imDim, imDim))
feat   = np.float32(feat)
labels = np.reshape(gt, (389880, 2))
'''
